...
class CardPredictor:
    ...
    def predict(self, car_pic, resize_rate=1):
        ...
        if blur > 0:
            ...
            # cv2.waitKey(0)
        oldimg = img
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)   #图像灰度化
        # cv2.imshow('GaussianBlur', img)
        ...
        # cv2.imshow('img_dege2', img_edge2)
        # cv2.waitKey(0)
        ...
        for i, color in enumerate(colors):
            if color in ("blue", "yellow", "green"):
                ...
                x_average = np.sum(x_histogram) / x_histogram.shape[0]  # 均值
                ...
                if len(wave_peaks) <= 6:
                    ...
                    continue
                part_cards = seperate_card(gray_img, wave_peaks)   # 分割牌照字符
                # 分割输出
                ...
                for i, part_card in enumerate(part_cards):
                    ...
                    part_card = cv2.copyMakeBorder(part_card, 0, 0, w, w, cv2.BORDER_CONSTANT, value=[0, 0, 0])  # 边缘填充
                    # cv2.imshow('part_card', part_card)
                    # cv2.waitKey(0)
                    part_card = cv2.resize(part_card, (SZ, SZ), interpolation=cv2.INTER_AREA)   # 图片缩放（缩小）
                    # cv2.imshow('part_card', part_card)
                    # cv2.waitKey(0)
